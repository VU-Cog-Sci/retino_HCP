...
instructions at the [bottom of this page](#using-retino_HCP-as-a-template).
...
`retino_HCP/retino_HCP.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `retino_HCP.py` in directory called
`retino_HCP`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import retino_HCP as sb` in an
interactive Python session, the classes and functions defined inside of the
`retino_HCP.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the retino_HCP module, the main classes defined at the bottom of
...
Either way, you can create a `retino_HCP/data` folder in which you can
...
(typically, something like our `retino_HCP/tests/test_retino_HCP.py`). Within each
...
have one such test in `retino_HCP/tests/test_retino_HCP.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/retino_HCP`),
and type:
...
will see a message such as:
    ...
    retino_HCP/tests/test_retino_HCP.py .F...
    ...
    retino_HCP/tests/test_retino_HCP.py:49: AssertionError
...
To document `retino_HCP` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/retino_HCP/blob/master/doc/_static/RTD-advanced-conf.png)

 http://retino_HCP.readthedocs.org/en/latest/
...
A `retino_HCP/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/retino_HCP) for the software. This
...
`retino_HCP` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `retino_HCP`, we use the
...
[here](https://github.com/uwescience/retino_HCP/blob/master/scripts/Figure1.ipynb).
...
### Using `retino_HCP` as a template
...
Under "old repository clone URL" enter: https://github.com/uwescience/retino_HCP
...
`retino_HCP/retino_HCP.py` to be called `smallish/smallish.py`

	git mv retino_HCP smallish
	git mv smallish/retino_HCP.py smallish/smallish.py
	git mv smallish/tests/test_retino_HCP.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `retino_HCP` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `retino_HCP` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `retino_HCP` in them. Type
the following to see where all these files are:

	git grep retino_HCP

You can replace `retino_HCP` for `smallish` quickly with:

	git grep -l 'retino_HCP' | xargs sed -i 's/retino_HCP/smallish/g'
...
Edit all the mentions of `retino_HCP` in the `retino_HCP/__init__.py` file, and in
the `retino_HCP/version.py` file as well.
