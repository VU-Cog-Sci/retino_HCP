...
python post_fit/extract_sum.py sub-01 gauss_sg
-----------------------------------------------------------------------------------------
"""
...
for type_data in ["gridfit","fit"]:
    ...
    # -------------------------
    summary_hdf5 = opj(base_dir,'pp_data',subject,fit_model,'h5',type_data)
    try: os.makedirs(summary_hdf5)
    except: pass
    summary_hdf5_file = opj(base_dir,'pp_data',subject,fit_model,'h5',type_data,'summary.h5')
    try: 
    ...
    # 4. median of r2 of the fit
    # 5. mean of r2 of the fit
    print('%s: summary processing of: %s'%(type_data,subject))
