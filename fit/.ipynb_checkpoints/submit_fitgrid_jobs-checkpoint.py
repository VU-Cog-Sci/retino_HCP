...
for iter_job in np.arange(0,start_idx.shape[0],1):
    ...
    opfn = opj(base_dir,'pp_data',subject,fit_model,'gridfit',base_file_name + '_est_%s_to_%s.gii' %(str(int(start_idx[iter_job])),str(int(end_idx[iter_job]))))
    ...
    js_name =  opj(base_dir, 'pp_data', subject, fit_model, 'jobs', '%s_vox_%s_to_%s_grid.sh'%(subject,str(int(start_idx[iter_job])),str(int(end_idx[iter_job]))))
