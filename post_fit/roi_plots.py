    ...
    for roi_num, roi in enumerate(analysis_info['rois']):
        ...
        for mask_dir in ['pos','neg']:    
            ...
            for hemi in ['L', 'R', 'LR']:
                # create folder
                roi_text = roi
                exec('fig_bokeh_dir_{mask_dir}_{hemi} = opj(base_dir,"pp_data",subject,fit_model,"figs","{type_data}","prf","{mask_dir}","{hemi}")'.\
                     format(mask_dir=mask_dir, hemi = hemi, type_data = type_data))
                try: exec('os.makedirs(fig_bokeh_dir_{mask_dir}_{hemi})'.format(mask_dir=mask_dir,hemi = hemi))
                except: pass

                if save_svg == 1:
                    # figure svg main folder
                    exec('fig_bokeh_dir_svg_{mask_dir}_{hemi} = opj(base_dir,"pp_data",subject,fit_model,"figs",{type_data}","svg","{mask_dir}","{hemi}")'.\
                         format(mask_dir=mask_dir, hemi = hemi, type_data = type_data))
