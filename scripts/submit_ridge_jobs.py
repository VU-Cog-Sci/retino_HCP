...
if 'lisa' in platform.uname()[1]:
    jobscript_template_file = os.path.join(os.getcwd(), 'lisa_jobscript_template_ridge.sh')
    base_dir = analysis_info['lisa_cluster_base_folder'] 
    sub_command = 'qsub '
    print('on lisa')
else:
    jobscript_template_file = os.path.join(os.getcwd(), 'cartesius_jobscript_template_ridge.sh')
