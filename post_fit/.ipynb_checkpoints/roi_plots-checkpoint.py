...
for type_rois in ['rois_subcortical','rois']:
    for roi_num, roi in enumerate(analysis_info[type_rois]):
                ...
                if hemi == 'LR':
                    ...
                    if hemi == 'L' or hemi == 'R':
                        if type_rois == 'rois_subcortical':
                            ...
                            roi_mask_file = opj(base_dir,'raw_data','surfaces','pauli_atlas',"CIT168toMNI152_prob_atlas_bilat_1mm_{roi}_resample.nii.gz".format(roi = roi))
                            nii_in_roi_mask = nb.load(roi_mask_file)
                            roi_mask_mat = nii_in_roi_mask.get_data()
                            roi_mask_mat_th = roi_mask_mat>=analysis_info['rois_subcortical_threshold']
                            mask_mat = np.logical_and(hemi_mask_mat,roi_mask_mat_th)
                            
                            # load time course of original data
                            orig_data_file  =  opj(base_dir,'raw_data',subject,'tfMRI_RETBARS_Atlas_1.6mm_MSMAll_hp2000_clean_sg_psc.dtseries_subcortical.nii.gz')
                            orig_data_file_load = nb.load(orig_data_file)
                            
                            orig_data = orig_data_file_load.get_data()
                            ...
                            # load time course of original data
                            orig_data_file  =  opj(base_dir,'raw_data',subject,'tfMRI_RETBARS_Atlas_1.6mm_MSMAll_hp2000_clean_sg_psc.dtseries_%s.func.gii'% hemi)
                            orig_data_file_load = nb.load(orig_data_file)
                            orig_data = []
