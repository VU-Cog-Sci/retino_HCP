...
# Functions import
# ----------------
from utils import set_pycortex_config_file, convert_fit_results, mask_gii_2_hdf5
...
if analysis_info['keep_svg'] == 0:
    try:
        overlays_file = sorted(glob.glob(opj(analysis_info['FS_subject_dir'], 'cortex', 'db', sub_id,'overlays.svg')))[0]
...
# figs_roi = opj(analysis_info["aeneas_project_directory"],"pp", sub_id, "figs_roi")                 # create figure folder
...
#     # create folder
#     exec('fig_roi_dir_{mask_dir} = opj(analysis_info["aeneas_project_directory"],"pp", sub_id, "figs_roi","{mask_dir}")'.format(mask_dir=mask_dir)) # create figure folder
#     try: exec('os.makedirs(fig_roi_dir_{mask_dir})'.format(mask_dir=mask_dir))                              # create folder
#     except: pass

#     # load data 
#     for data_name in data_names:
#         exec('{data_name}_f = opj(deriv_dir_{mask_dir},"loo_prf_{data_name}_{mask_dir}.nii.gz")'.format(mask_dir=mask_dir,data_name = data_name)) # define data to load
...
#         exec('pl.savefig(opj(fig_roi_dir_{mask_dir}, "loo_{vol_name}_{mask_dir}.pdf"),facecolor="w")'.format(mask_dir=mask_dir,vol_name = vol_name))
...
#     exec('dataset_webgl.save(opj(fig_roi_dir_{mask_dir}, dataset_name))'.format(mask_dir = mask_dir)) # saving dataset
#     pl.close()
