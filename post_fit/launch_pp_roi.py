    ...
    if os.path.isdir(opj(base_dir,'pp_data',subject,fit_model,'fit')):
        ...
        for hemi in ['L','R']:
            for iter_job in np.arange(0,start_idx.shape[0],1):
                fit_est_file = opj(base_dir,'pp_data',subject,fit_model,'fit', '%s_%s.func_bla_psc_est_%s_to_%s.gii' %\ 
                                                           (base_file_name,hemi,str(int(start_idx[iter_job])),str(int(end_idx[iter_job]))))
...
for subject_fit in subject_fit_list:
    if num_run < max_tmux:
        if os.path.isdir(opj(base_dir,'pp_data',subject_fit,fit_model,'figs','roi'))==0:
            ...
            print("tmux new-session -d -s {session_name} 'python post_fit/pp_roi.py {subject_fit} {fit_model} {job_vox}'".format(\
                session_name = session_name, subject_fit = subject_fit, fit_model = fit_model, job_vox = job_vox))
            os.system("tmux new-session -d -s {session_name} 'python post_fit/pp_roi.py {subject_fit} {fit_model} {job_vox}'".format(\
                session_name = session_name, subject_fit = subject_fit, fit_model = fit_model, job_vox = job_vox))
            time.sleep(2)
            num_run = num_run + 1